...
log_line log::add( net_str topic, int level )
{
...
log_line::log_line( net_str topic, int lvl )
: is_first_( true )
{
  char tbuf[32];
...
void log_line::add_key( net_str key )
{
  if ( !is_first_ ) {
    std::cerr << ',';
  } else {
...
log_line& log_line::add( net_str key, net_str val )
{
  add_key( key );
...
log_line& log_line::add( net_str key, int val )
{
  add_key( key );
  std::cerr << val;
  return *this;
...
log_line& log_line::add( net_str key, int64_t val )
{
  add_key( key );
  std::cerr << val;
  return *this;
...
log_line& log_line::add( net_str key, double val )
{
  add_key( key );
  std::cerr << val;
  return *this;
