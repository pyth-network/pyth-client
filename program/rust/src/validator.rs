use {
    crate::{
        accounts::{
            ...
            PriceAccount,
            PythAccount,
        }, c_oracle_header::PC_MAGIC, error::OracleError, processor::{c_upd_aggregate, c_upd_twap}, utils::pyth_assert
    },
...
fn validate_price_account(
    ...
    Some(bytemuck::from_bytes_mut::<PriceAccount>(&mut price_account_info[0..size_of::<PriceAccount>()]))
}
...
fn update_aggregate(
    ...
    let updated =
        unsafe { c_upd_aggregate(price_account as *mut PriceAccount as *mut u8, slot, timestamp) };
...
pub fn aggregate_price(
    ...
    if let Some(price_account) = validate_price_account(price_account_data) {
