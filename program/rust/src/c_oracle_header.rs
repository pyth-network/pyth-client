...
pub fn to_u32_checked<T: ToPrimitive>(arg: &T) -> Result<u32, OracleError> {
    arg.to_u32().ok_or(OracleError::EnumConversionError)
}
...
impl PythAccount for MappingAccount {
    const ACCOUNT_TYPE: AccountType = AccountType::Mapping;
    const INITIAL_SIZE: u32 = 56; // Tested in test_sizes.rs
}

impl PythAccount for ProductAccount {
    const ACCOUNT_TYPE: AccountType = AccountType::Product;
