...
use crate::c_oracle_header::{cmd_hdr_t, cmd_upd_product_t, command_t_e_cmd_upd_product, pc_prod_t, PC_PROD_ACC_SIZE, PC_VERSION, pc_price_t, cmd_set_min_pub_t};
use crate::deserialize::load_mut;
use crate::rust_oracle::{initialize_checked, load_checked, read_pc_str_t, set_min_pub, try_convert, upd_product};
...
fn test_set_min_pub() {
  ...
  assert!(upd_product(
    ...
    .is_ok());
  assert!(account_has_key_values(&price_account, &kvs).unwrap_or(false));
