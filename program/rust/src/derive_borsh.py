...
BORSH_TRAITS = "BorshSerialize, BorshDeserialize, "
USE_BORSH = "use borsh::{BorshDeserialize, BorshSerialize};"
...
for struct_name in sys.argv[2:]:
    ...
    if "derive" not in content[line_num - 1]:
        ...
        if BORSH_TRAITS in derive_line:
            continue
        traits_start = content[line_num - 1].index("(") + 1
        new_derive_line = derive_line[:traits_start] + BORSH_TRAITS + derive_line[traits_start:]
        content[line_num - 1] = new_derive_line

#check if we need to add use borsch
if content[0] != USE_BORSH:
    content.insert(0, USE_BORSH)
