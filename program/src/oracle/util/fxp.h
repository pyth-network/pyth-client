...
/* fxp_exp2_approx computes:

     y/2^30 ~ exp2( x/2^30 )

   with an error of O(1) ulp for x/2^30 < ~1.  This works identical to
   exp2m1.h but uses.  As implemented this is +/-1 ulp of the correctly
   rounded RNE result when x<=2^30, has the leading ~30 bits correct for
   larger x and is and exact for input values that yield exactly
   representable outputs.  Returns UINT64_MAX if output would overflow a
   34.30u input. */

static inline uint64_t
fxp_exp2_approx( uint64_t x ) {
  uint64_t i = x >> 30;
  if( i>=UINT64_C(34) ) return UINT64_MAX;
  uint64_t d = x & ((UINT64_C(1)<<30)-UINT64_C(1));
  uint64_t y;
  /* BEGIN AUTOGENERATED CODE */
  /* bits 33.4 rms_aerr 4.7e-11 rms_rerr 3.5e-11 max_aerr 9.9e-11 max_rerr 9.0e-11 */
  y = UINT64_C(0x00000002d6e5bd1d);
  y = UINT64_C(0x0000000257992e8b) + ((y*d)>>33);
  y = UINT64_C(0x00000002c02265a3) + ((y*d)>>33);
  y = UINT64_C(0x000000027607eb13) + ((y*d)>>33);
  y = UINT64_C(0x00000001c6b30b08) + ((y*d)>>33);
  y = UINT64_C(0x00000001ebfbcc25) + ((y*d)>>32);
  y = UINT64_C(0x00000002c5c8604f) + ((y*d)>>31);
  y =                                 (y*d);
  /* END AUTOGENERATED CODE */
  int s = 34-(int)i;
  return ((y+(UINT64_C(1) << (s-1))) >> s) + (UINT64_C(1)<<(64-s));
}

/* fxp_rexp2_approx computes:
...
static inline uint64_t
fxp_rexp2_approx( uint64_t x ) {
  uint64_t i = x >> 30;
  if( i>=UINT64_C(31) ) return UINT64_C(0);
  uint64_t d = x & ((UINT64_C(1)<<30)-UINT64_C(1));
  uint64_t y;
  /* BEGIN AUTOGENERATED CODE */
  /* bits 35.4 rms_aerr 2.4e-11 rms_rerr 1.4e-11 max_aerr 3.3e-11 max_rerr 2.2e-11 */
  y = UINT64_C(0x00000002d6e2c6a2);               /* scale 50 bout 34 bmul  - */
  y = UINT64_C(0x0000000269e37ccb) - ((y*d)>>34); /* scale 46 bout 34 bmul 64 */
  y = UINT64_C(0x00000002b83379a8) - ((y*d)>>33); /* scale 43 bout 34 bmul 64 */
  y = UINT64_C(0x00000002762c0cea) - ((y*d)>>33); /* scale 40 bout 34 bmul 64 */
  y = UINT64_C(0x00000001c6af4b81) - ((y*d)>>33); /* scale 37 bout 34 bmul 64 */
  y = UINT64_C(0x00000001ebfbd6aa) - ((y*d)>>32); /* scale 35 bout 34 bmul 64 */
  y = UINT64_C(0x00000002c5c85fb1) - ((y*d)>>31); /* scale 34 bout 34 bmul 64 */
  y = UINT64_C(0x8000000000000000) - ((y*d)>> 1); /* scale 63 bout 64 bmul 64 */
  /* END AUTOGENERATED CODE */
  int s = 33+(int)i;
