...
// Not whole PC_NUM_COMP_PYTHNET because of stack issues appearing in
// upd_aggregate()
...
// The biggest instruction appears to be about ~10300 CUs, so overestimate by
// 100%.
#define CU_BUDGET_PER_IX 20000

// binary version of sysvar_clock account id
const uint64_t sysvar_clock[] = {0xc974c71817d5a706UL, 0xb65e1d6998635628UL,
...
const uint64_t compute_budget_program_id[] = {
    0x321721e56f460603UL, 0xe79bc372baadecffUL, 0x6b12f7c5bbe58cbcUL,
    0x403a9b432cUL};
...
typedef struct pc_price {
  ...
  int8_t   message_sent_; // flag to indicate if the current aggregate price has
                          // been sent as a message to the message buffer, 0 if
                          // not sent, 1 if sent
  uint8_t max_latency_;   // configurable max latency in slots between send and
                          // receive
  ...
  int64_t  prev_price_; // aggregate price of previous aggregate with TRADING
                        // status
  uint64_t prev_conf_;  // confidence interval of previous aggregate with
                        // TRADING status
  int64_t prev_timestamp_; // unix timestamp of previous aggregate with
                           // TRADING status
...
/*
The value 240 is derived from the fixed size of the pc_price_t struct excluding
the size of the comp_ array. Here is the breakdown of the sizes (in bytes) for
each field within the pc_price_t struct:
...
- prod_ (pc_pub_key_t): 32 bytes (assuming pc_pub_key_t is a 32-byte array or
struct)
...
Adding up all these sizes gives us a total of 240 bytes for the fixed part of
the pc_price_t struct. The size of the comp_ array is variable and depends on
PC_NUM_COMP, hence it is calculated separately and added to the base size of 240
bytes.
*/
#define PC_EXPECTED_PRICE_T_SIZE_PYTHNET                                       \
  (240 + PC_NUM_COMP * sizeof(pc_price_comp_t))

static_assert(sizeof(pc_price_t) == PC_EXPECTED_PRICE_T_SIZE_PYTHNET, "");
#undef PC_EXPECTED_PRICE_T_SIZE_PYTHNET

// This constant needs to be an upper bound of the price account size, it is
// used within pythd for ztsd. It is set tighly to the current price account +
// 96 component prices + 48 bytes for cumulative sums
...
typedef enum {
  ...
  // publish component price, never returning an error even if the update
  // failed key[0] funding account       [signer writable] key[1] price
  // account         [writable] key[2] sysvar_clock account  [readable]
...
{
  return p1->k8_[0] == p2->k8_[0] && p1->k8_[1] == p2->k8_[1] &&
         p1->k8_[2] == p2->k8_[2] && p1->k8_[3] == p2->k8_[3];
}

// check for null (zero) public key
inline bool pc_pub_key_is_zero(pc_pub_key_t *p)
{
  return p->k8_[0] == 0UL && p->k8_[1] == 0UL && p->k8_[2] == 0UL &&
         p->k8_[3] == 0UL;
}
